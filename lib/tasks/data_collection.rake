namespace :data_collection do
  ...
  task get_cards: :environment do
    ...
    puts starting_card_multiverse_id
  ...
  task get_all_cards: :environment do
    require 'open-uri'

    # Card ids
    path = "http://api.mtgdb.info/cards"
    buffer = open(path).read

    cards = JSON.parse buffer

    cards.each do |c|
      # Check if card exists
      if Card.find_by_multiverse_id(c["id"])
        puts "Card exists."
        puts c["id"]
        next
      end
